...
namespace zushisa9tt::blehomecentral {
    ...
    ble_central::ble_central(int fd_sighup, /* OBSOLETE int fd_write,*/  std::unique_ptr<api_task_queue>& t_q) :
     _task_queue(t_q) {
        _fds_poll[0].fd = fd_sighup;
        _fds_poll[0].events = POLLIN;

        // OBSOLETE _fd_write = fd_write;
                ...
                if (_fds_poll[1].revents & POLLIN) {
                    ...
                    if (check_report_filter(f_type, adv_info)) {
                        ...
                        if (tphdata->is_valid(wh_list))	{	// has BME280 data.
                            ...
                            _task_queue->push_task(jdata);
                            /* OBSOLETE if (write(_fd_write, jdata.c_str(), jdata.size()) < 0) {
                                char msgbuff[128] = {0};
                                strerror_r(errno, msgbuff, 128);
                                DEBUG_PRINTF("BLE: WRITE ERROR OCCURRED. %s\n", msgbuff);
                                throw tt_ble_exception(string("write error. ") + msgbuff);
                            }*/
                            tt_logger::instance().printf("BLE[INFO]: push data to the task queue => %s\n", jdata.c_str());
                        }
                    }
    ...
    void ble_central::start_hci_scan(tph_datastore& datastore, std::set<std::string>& wh_list) {
        ...
        uint16_t interval = htobs(0x2000/*0x0010*/);		// N*0.625ms => 10240ms(N:0x4000), 5120ms(N:0x2000), 2560ms(N:0x1000)
